...
import { exchangeMetaCode, listMetaAdAccounts } from "@/lib/integrations/meta";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (accounts.length === 0) throw new Error("Nenhuma conta Meta Ads acessivel.");
    ...
    return NextResponse.redirect(
      new URL(`/clients/${state.clientId}?integration=meta`, request.url),
    );
  } catch {
    return NextResponse.redirect(
      new URL(`/clients/${state.clientId}?integrationError=meta`, request.url),
    );
  }
}
